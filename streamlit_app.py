...
if not st.session_state.logged_in:
    ...
    if proc:
        ...
        for r in proc:
            ...
            c = st.columns(3)
            if c[0].button("✅ COMPLETE", key=f"d_{sid}"): global_db.setdefault(sid, {})[team] = "Done"; st.rerun()
    ...
    if hold:
        ...
        for r in hold:
            ...
            st.markdown(draw_combined_row(r, sid), unsafe_allow_html=True)
            c = st.columns(2)
            if c[0].button("▶ RESUME", key=f"re_{sid}"): global_db.setdefault(sid, {})[team] = "In Process"; st.rerun()
            if c[1].button("🔙 QUEUE", key=f"q2_{sid}"): global_db.setdefault(sid, {})[team] = "Pending"; st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    # --- UPDATED MASTER EXPORT (11 COLUMN FORMAT) ---
    st.write("---")
    master_rows = []
    for _, row in df_all.iterrows():
        ...
        # Populate columns for each of the 11 preferences
        for i, col_name in enumerate(pref_cols, 1):
            target_team = normalize_team(row[col_name])
            if target_team:
                current_stat = global_db.get(sid, {}).get(target_team, "Pending")
